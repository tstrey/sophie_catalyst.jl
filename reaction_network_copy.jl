...
macro continuous_event(ex::Expr)
    if !isvalidexpr($ex)
        throw(ArgumentError("Invalid expression for continuous_event macro: $ex"))
    end
    return esc(ModelingToolkit.continuous_events(ex))
end

macro discrete_event(ex::Expr)
    if !isvalidexpr($ex)
        throw(ArgumentError("Invalid expression for discrete_event macro: $ex"))
    end
    #return esc(:($(QuoteNode(ModelingToolkit.continuous_events($(QuoteNode(ex)))))))
    return esc(ModelingToolkit.discrete_events(ex))
end
...
macro reaction_network(ex::Expr)
    ...
    # Check if @discrete_event or @continuous_event definitions are present
    discrete_event_exprs = Expr(:block)
    continuous_event_exprs = Expr(:block)
    discrete_event_definitions = []
    continuous_event_definitions = []
    
    for ex_item in ex.args
        if ex_item.head == :macrocall
            if ex_item.args[1] == :@discrete_event
                push!(discrete_event_definitions, ex_item.args[2:end]...)
            elseif ex_item.args[1] == :@continuous_event
                push!(continuous_event_definitions, ex_item.args[2:end]...)
            else
                push!(discrete_event_exprs.args, ex_item)
            end
        else
            push!(discrete_event_exprs.args, ex_item)
        end
    ...
    # Create the reaction network block with discrete and continuous event definitions
    reaction_network_block = Expr(:block, :(@parameters t), discrete_event_exprs, continuous_event_exprs)

    # If discrete event definitions are present, add them to the block
    if !isempty(discrete_event_definitions)
        push!(reaction_network_block.args, :(@discrete_events), discrete_event_definitions...)
    end
    
    # If continuous event definitions are present, add them to the block
    if !isempty(continuous_event_definitions)
        push!(reaction_network_block.args, :(@continuous_events), continuous_event_definitions...)
    end

    make_reaction_system(reaction_network_block)
end
